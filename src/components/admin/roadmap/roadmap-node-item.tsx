...
import { Circle, CircleDot, CheckCircle, Star, ChevronUp, ChevronDown, Pencil, Trash2, Plus } from 'lucide-react'
import { toast } from 'sonner'
import { updateNodeStatus, deleteNode, reorderNodes } from '@/lib/actions/roadmap-actions'
...
function StatusIcon({ status }: { status: RoadmapNodeStatusType }) {
  switch (status) {
    case 'not_started':
...
// Simplified item for child nodes (no nesting below level 2)
type ChildProps = {
  node: RoadmapNode
  roadmapId: string
  rootNodes: RoadmapNode[]
...
function ChildNodeItem({ node, roadmapId, rootNodes, isFirst, isLast, onStatusChange, onMutated }: ChildProps) {
  const [isPending, startTransition] = useTransition()
  const [editOpen, setEditOpen] = useState(false)
  const [confirmDelete, setConfirmDelete] = useState(false)
